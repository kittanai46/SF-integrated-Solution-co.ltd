...
import 'package:crimes/data/client/request/outer/alienno_request_dto.dart';
...
class ApiClient with ApiResultExtractor {
  ...
  AllDatabase getListDB(String type, UserDto userDto, ParamScope paramScope, bool fromFavorite) {
    ...
    switch (type) {
      case AppConstant.kSearchPerson: {
        ...
        if (userDto.roleSearchExternal!.flagPersonTm!) {
          if (paramScope.hasInputAlienNo || paramScope.hasInputNames) {
            outerDbArray.add(DatabaseDto(
                title: AppConstant.msOuterPersonMigwis,
                category: AppConstant.msOuterPersonMigwis, enable: true));
          } else if(!fromFavorite){
            outerDbArray.add(DatabaseDto(
                title: AppConstant.msOuterPersonMigwis,
                category: AppConstant.msOuterPersonMigwis,
                enable: false,hintKeyword: "โปรดระบุ ชื่อ-สกุล หรือ หมายเลขทะเบียนแรงงาน"));
          }
        }
        ...
        if (userDto.roleSearchExternal!.flagPersonDopaLinkage!) {
          if (paramScope.hasInputPid) {
            ...
            outerDbArray.add(DatabaseDto(title: AppConstant.msOuterPersonMigwis,
                category: AppConstant.msOuterPersonMigwis, enable: true));
          } else if(!fromFavorite){
            outerDbArray.add(DatabaseDto(title: AppConstant.msOuterPersonAlien,
            ...
            outerDbArray.add(DatabaseDto(title: AppConstant.msOuterPersonMigwis,
                category: AppConstant.msOuterPersonMigwis, enable: false,hintKeyword: "โปรดระบุ หมายเลขทะเบียนแรงงาน"));
          }
        }
  ...
  //Migwis ----------------------------------------------------
  Future<MigwisResponseDto> searchMigwisByAlienNo(String alienNo, double lat, double lon) async {
    ServiceApi serviceApi = await ClientBuilder().createService();

    final dto = AlienNoRequestDto(alien_no: alienNo,lat:lat,lon:lon);
    EncryptRequestDto encryptDto = await CryptoUtils.encrypt(dto);
    final encryptResponseDto = await serviceApi.searchMigwis(encryptDto);

    final MigwisResponseDto response = MigwisResponseDto();
    if(encryptResponseDto.data!=null && encryptResponseDto.data.isNotEmpty
        && encryptResponseDto.nonce!=null && encryptResponseDto.nonce.isNotEmpty){
      String jsonString = await CryptoUtils.decrypt(encryptResponseDto.data, encryptResponseDto.nonce);
      StringUtils.log("searchMigwisByAlienNo: $jsonString");

      final listExtensionDto = ListMigwisDto.fromJson(jsonDecode(jsonString));

      response.status = encryptResponseDto.status;
      response.message = encryptResponseDto.message;
      response.listMigwisDto = listExtensionDto;
      StringUtils.log("searchMigwisByAlienNo success");
    }else{
      response.status = encryptResponseDto.status;
      response.message = encryptResponseDto.message;
      response.listMigwisDto = ListMigwisDto(data: []);
      StringUtils.log("searchMigwisByAlienNo failed");
    }
    return response;
  }

  Future<MigwisResponseDto> searchMigwisByName(String firstName,String lastName, double lat, double lon) async {
    ServiceApi serviceApi = await ClientBuilder().createService();
    ...
    if(encryptResponseDto.data!=null && encryptResponseDto.data.isNotEmpty
        && encryptResponseDto.nonce!=null && encryptResponseDto.nonce.isNotEmpty){
      String jsonString = await CryptoUtils.decrypt(encryptResponseDto.data, encryptResponseDto.nonce);
      StringUtils.log("searchMigwisByName: $jsonString");

      final listMigwisDto = ListMigwisDto.fromJson(jsonDecode(jsonString));

      response.status = encryptResponseDto.status;
      response.message = encryptResponseDto.message;
      response.listMigwisDto = listMigwisDto;
      StringUtils.log("searchMigwisByName success");
    }else{
      response.status = encryptResponseDto.status;
      response.message = encryptResponseDto.message;
      response.listMigwisDto = ListMigwisDto(data: []);
      StringUtils.log("searchMigwisByName failed");
    }
    return response;
  }
