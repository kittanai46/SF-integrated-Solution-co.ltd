...
class _SearchDetailScreenState extends State<SearchDetailScreen> {
  ...
  Future<void> _downloadWarrant() async {
    if (_viewModel.warrantDto == null ||
        _viewModel.warrantDto!.wrFileId == null ||
        _viewModel.warrantDto!.wrFileId!.isEmpty) return;

    String warrantFileId = _viewModel.warrantDto!.wrFileId!;
    String downloadUrl =
        "${AppConstant.apiUrl}${AppConstant.downloadWarrantPath}${warrantFileId}";
    ...
    Fluttertoast.showToast(
        msg: "ดาวน์โหลดไฟล์หมายจับเรียบร้อยแล้ว",
  ...
  void _openPdf(String url) {
    ...
    ScreenNavigator.navigateTo(Routes.searchPdf,
        param: SearchPdfScreenParam(_viewModel.userDto!, url));
  }
  ...
  Future<void> _downloadWarrantCRDBase64() async {
    if (_viewModel.warrantCRDDto == null ||
        _viewModel.warrantCRDDto!.warrant_doc == null ||
        _viewModel.warrantCRDDto!.warrant_doc!.isEmpty) return;
  ...
  Future<void> _downloadWarrantCourtCRDBase64() async {
    if (_viewModel.warrantCourtCRDDto == null ||
        _viewModel.warrantCourtCRDDto!.warrant_doc == null ||
        _viewModel.warrantCourtCRDDto!.warrant_doc!.isEmpty) return;
  ...
  String _getFormTitle() {
    ...
    switch (_viewModel.category) {
      ...
      case AppConstant.dbOuterPersonPrisoner:
        ...
        break;
      case AppConstant.dbOuterPersonWarrantCourt:
        title = AppConstant.msOuterPersonWarrantCourt;
        break;
        ...
        builder: (context) => Scaffold(
            ...
            body: Center(
              child: StatusPopup(
                  ...
                  onPress: () {
                    Navigator.of(context).pop();
                    ScreenNavigator.replaceEntireStackTo(Routes.main,
                        param: MainScreenParam(_viewModel.userDto!));
                  }),
            )));
  }
        ...
        builder: (context) => Scaffold(
            ...
            body: Center(
              child: StatusPopup(
                  ...
                  description:
                      'โปรดเปิดการใช้งานตำแหน่ง GPS ของอุปกรณ์ ก่อนการสืบค้น',
                  // cancelText: AppMessage.cancel,
                  // onCancel: () {
                  //   Navigator.of(context).pop();
                  // },
  ...
  Future<void> _onScrollToBottom() async {
    ...
    if (_viewModel.category == AppConstant.dbInnerPersonHasAya &&
        _viewModel.warrantDto != null &&
        _viewModel.warrantDto!.orgCode != null) {
      // _buildWarrantCase()
      if (!AppConstant.isMockUser &&
          (_viewModel.warrantDto != null &&
              _viewModel.warrantDto!.orgCode != null &&
              _viewModel.warrantDto!.orgCode!.isNotEmpty)) {
        if (_viewModel.warrantDto!.caseType == 'อาญา' ||
            _viewModel.warrantDto!.caseType == 'คดีอาญา') {
          ...
          if (_viewModel.crimeCaseDto != null) {
            Fluttertoast.showToast(
                msg: "โหลดข้อมูล 'คดี' เพิ่มเติมเรียบร้อย",
                toastLength: Toast.LENGTH_LONG,
                gravity: ToastGravity.BOTTOM);
          }
        } else {
    ...
    //
    // }
    else if (_viewModel.category == AppConstant.dbOuterPersonCivil &&
        _viewModel.personDto != null &&
        _viewModel.personDto!.pID != null &&
        _viewModel.personDto!.pID!.isNotEmpty) {
      // _buildPersonDetail()
      ...
      if (!AppConstant.isMockUser &&
          (_viewModel.houseDto == null ||
              _viewModel.houseDto!.houseID == null ||
              _viewModel.houseDto!.houseID!.isEmpty)) {
        if (_nowRequestHouse) return;
        _nowRequestHouse = true;
        await _viewModel.requestHouse();
        if (_viewModel.houseDto != null) {
          Fluttertoast.showToast(
              msg: "โหลดข้อมูล 'ที่อยู่' เพิ่มเติมเรียบร้อย",
              toastLength: Toast.LENGTH_LONG,
              gravity: ToastGravity.BOTTOM);
        }
      }
    ...
    //
    // }
    else if (_viewModel.category == AppConstant.dbOuterPersonLinkage &&
        _viewModel.personLkDto != null &&
        _viewModel.personLkDto!.pid != null &&
        _viewModel.personLkDto!.pid!.isNotEmpty) {
      ...
      // }
      if (!AppConstant.isMockUser &&
          (_viewModel.makeCardLkDto == null ||
              _viewModel.makeCardLkDto!.documentNumber == null ||
              _viewModel.makeCardLkDto!.documentNumber!.isEmpty)) {
        ...
        if (_viewModel.makeCardLkDto != null) {
          Fluttertoast.showToast(
              msg: "โหลดข้อมูล 'การทำบัตร' เพิ่มเติมเรียบร้อย",
              toastLength: Toast.LENGTH_LONG,
              gravity: ToastGravity.BOTTOM);
        }
      }
      if (!AppConstant.isMockUser &&
          (_viewModel.houseLkDto == null ||
              _viewModel.houseLkDto!.houseID == null ||
              _viewModel.houseLkDto!.houseID!.isEmpty)) {
    ...
    } else if (_viewModel.category == AppConstant.dbOuterPersonAlien &&
        _viewModel.alienPersonDto != null &&
        _viewModel.alienPersonDto!.personalID != null &&
        _viewModel.alienPersonDto!.personalID!.isNotEmpty) {
      if (!AppConstant.isMockUser &&
          (_viewModel.houseLkDto == null ||
              _viewModel.houseLkDto!.houseID == null ||
              _viewModel.houseLkDto!.houseID!.isEmpty)) {
    ...
    } else if (_viewModel.category == AppConstant.dbOuterPersonSocialEmployee &&
        _viewModel.employmentDto != null &&
        _viewModel.employmentDto!.ssoNum != null &&
        _viewModel.employmentDto!.ssoNum!.isNotEmpty) {
      // _buildEmploymentDetail()
      if (_viewModel.hospitalDto == null ||
          _viewModel.hospitalDto!.hospitalCode == null ||
          _viewModel.hospitalDto!.hospitalCode!.isEmpty) {
        if (_nowRequestHospital) return;
        _nowRequestHospital = true;
        await _viewModel.requestHospital();
        if (_viewModel.hospitalDto != null) {
          Fluttertoast.showToast(
              msg: "โหลดข้อมูล 'สถานพยาบาล' เพิ่มเติมเรียบร้อย",
              toastLength: Toast.LENGTH_LONG,
              gravity: ToastGravity.BOTTOM);
        }
      }
            ...
            Expanded(
              child: Container(
                ...
                child: Column(
                  children: [
                    _buildMainContent(),
                    if (!AppConstant.isMockUser) _buildButtons(),
                  ],
                ),
              ),
            ),
            ...
            Material(
              ...
              child: InkWell(
                ...
                child: Container(
                  width: 45.w,
                  child: Center(
                    child: Image.asset(AppImage.ic_home,
                        fit: BoxFit.contain, width: 24.w, height: 45.h),
                  ),
                ),
              ),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if (_viewModel.category == AppConstant.dbOuterPersonCivil &&
                    _viewModel.personDto != null &&
                    _viewModel.personDto!.pID != null &&
                    _viewModel.personDto!.pID!.isNotEmpty)
                  _buildIdcImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonPrisoner &&
                    _viewModel.prisonerDto != null &&
                    _viewModel.prisonerDto!.image != null &&
                    _viewModel.prisonerDto!.image!.isNotEmpty &&
                    _viewModel.prisonerDto!.prisonerId != null &&
                    _viewModel.prisonerDto!.prisonerId!.isNotEmpty)
                  _buildPrisonerDetail(),
                if (_viewModel.category == AppConstant.dbOuterPersonLinkage &&
                    _viewModel.personLkDto != null &&
                    _viewModel.personLkDto!.pid != null &&
                    _viewModel.personLkDto!.pid!.isNotEmpty)
                  _buildPersonLkImage(),
                if (_viewModel.category == AppConstant.dbInnerPersonFaceRec &&
                    _viewModel.personLkDto != null &&
                    _viewModel.personLkDto!.pid != null &&
                    _viewModel.personLkDto!.pid!.isNotEmpty)
                  _buildPersonLkImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonForeign &&
                    _viewModel.workerDto != null &&
                    _viewModel.workerDto!.workerID != null &&
                    _viewModel.workerDto!.workerID!.isNotEmpty)
                  _buildWorkerDetail(),
                if (_viewModel.category == AppConstant.dbOuterPersonAlien &&
                    _viewModel.alienPersonDto != null &&
                    _viewModel.alienPersonDto!.personalID != null &&
                    _viewModel.alienPersonDto!.personalID!.isNotEmpty)
                  _buildAlienPersonImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonWorker &&
                    _viewModel.alienWorkerDto != null &&
                    _viewModel.alienWorkerDto!.personalID != null &&
                    _viewModel.alienWorkerDto!.personalID!.isNotEmpty)
                  _buildAlienWorkerImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonNonThai &&
                    _viewModel.personNonThaiDto != null &&
                    _viewModel.personNonThaiDto!.personalID != null &&
                    _viewModel.personNonThaiDto!.personalID!.isNotEmpty)
                  _buildPersonNonThaiImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonNoRegis &&
                    _viewModel.personNoRegisDto != null &&
                    _viewModel.personNoRegisDto!.personalID != null &&
                    _viewModel.personNoRegisDto!.personalID!.isNotEmpty)
                  _buildPersonNoRegisImage(),
                if (_viewModel.category == AppConstant.dbOuterPersonExtension &&
                    _viewModel.extensionDto != null &&
                ...
                if (_viewModel.category == AppConstant.dbInnerPersonHasAya &&
                    ...
                    _viewModel.warrantDto!.orgCode != null)
                  _buildWarrantCase(),
                if (_viewModel.category ==
                        AppConstant.dbInnerPersonWarrantCRD &&
                    _viewModel.warrantCRDDto != null &&
                    _viewModel.warrantCRDDto!.warrant_doc != null &&
                    _viewModel.warrantCRDDto!.warrant_doc!.isNotEmpty)
                  _buildWarrantCRD(),
                if (_viewModel.category ==
                        AppConstant.dbInnerPersonWarrantCourtCRD &&
                    _viewModel.warrantCourtCRDDto != null &&
                    _viewModel.warrantCourtCRDDto!.warrant_doc != null &&
                    _viewModel.warrantCourtCRDDto!.warrant_doc!.isNotEmpty)
                ...
                if (_viewModel.category ==
                        AppConstant.dbOuterPersonSocialEmployee &&
                    _viewModel.employmentDto != null &&
                    _viewModel.employmentDto!.ssoNum != null &&
                    _viewModel.employmentDto!.ssoNum!.isNotEmpty)
                  _buildEmploymentDetail(),
                // if(_viewModel.category == AppConstant.dbOuterPersonImmigration && _viewModel.travelDto != null && _viewModel.travelDto!.tMRunNo != null && _viewModel.travelDto!.tMRunNo!.isNotEmpty)
                //   _buildTravelDetail(),
                if (_viewModel.category == AppConstant.dbInnerCarAya &&
                    _viewModel.crimeCaseVehicleDto != null &&
                    _viewModel.crimeCaseVehicleDto!.orgCode != null)
                  _buildVehicleCrimeCase(),
                if (_viewModel.category == AppConstant.dbInnerCarTraffic &&
                    _viewModel.trafficCaseVehicleDto != null &&
                    _viewModel.trafficCaseVehicleDto!.orgCode != null)
  ...
  Widget _buildActionTitle(
          String title, Function action, Function isExpanded) =>
      Container(
        padding: EdgeInsets.symmetric(vertical: 16.h),
        child: Material(
          color: Colors.transparent,
                  ...
                  Expanded(
                    child: Row(
                        ...
                        children: [
                          ...
                          (isExpanded())
                              ? Image.asset(AppImage.ic_chevron_right,
                                  fit: BoxFit.contain)
                              : Image.asset(AppImage.ic_chevron_down,
                                  fit: BoxFit.contain),
                        ]),
                  ),
                ])),
  ...
  Widget _buildIdcDetail() => Container(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personDto != null &&
                _viewModel.personDto!.pID != null &&
                _viewModel.personDto!.pID!.isNotEmpty)
              //_buildTitle("ข้อมูลบัตร"),
              _buildActionTitle(
                  "ข้อมูลบัตรล่าสุด", _viewModel.requestIdc, isExpandIdcDetail),
  ...
  Widget _buildIdcImage() => Container(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personImageDto != null &&
                _viewModel.personImageDto!.image != null &&
                _viewModel.personImageDto!.image!.isNotEmpty &&
                _viewModel.personImageBytes != null)
              Container(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.personImageDto == null ||
                    _viewModel.personImageDto!.image == null ||
                    _viewModel.personImageDto!.image!.isEmpty ||
                    _viewModel.personImageBytes == null ||
                    _viewModel.personImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  bool isExpandIdcDetail() {
    return (_viewModel.idcDto != null &&
        _viewModel.idcDto!.pID != null &&
        _viewModel.idcDto!.pID!.isNotEmpty);
  }

  bool isExpandHouseDetail() {
    return (_viewModel.houseDto != null &&
        _viewModel.houseDto!.houseID != null &&
        _viewModel.houseDto!.houseID!.isNotEmpty);
  }
  // --- END Civil ------
  ...
  Widget _buildWorkerDetail() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.workerImageDto != null &&
                _viewModel.workerImageDto!.image != null &&
                _viewModel.workerImageDto!.image!.isNotEmpty &&
                _viewModel.workerImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.workerImageDto == null ||
                    _viewModel.workerImageDto!.image == null ||
                    _viewModel.workerImageDto!.image!.isEmpty ||
                    _viewModel.workerImageBytes == null ||
                    _viewModel.workerImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildPrisonerDetail() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.prisonerImageDto != null &&
                _viewModel.prisonerImageDto!.image != null &&
                _viewModel.prisonerImageDto!.image!.isNotEmpty &&
                _viewModel.prisonerImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.prisonerImageDto == null ||
                    _viewModel.prisonerImageDto!.image == null ||
                    _viewModel.prisonerImageDto!.image!.isEmpty ||
                    _viewModel.prisonerImageBytes == null ||
                    _viewModel.prisonerImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildPersonLkDetail() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personLkDto != null &&
                _viewModel.personLkDto!.pid != null &&
                _viewModel.personLkDto!.pid!.isNotEmpty)
              _buildActionTitle(
                  "ข้อมูลที่อยู่", _viewModel.requestHouseLk, isExpandHouseLk),
            if (_viewModel.houseLkDto != null &&
                _viewModel.houseLkDto!.houseID != null &&
                _viewModel.houseLkDto!.houseID!.isNotEmpty)
            ...
            if (_viewModel.personLkDto != null &&
                _viewModel.personLkDto!.pid != null &&
                _viewModel.personLkDto!.pid!.isNotEmpty)
              _buildActionTitle("ข้อมูลบัตรล่าสุด",
                  _viewModel.requestMakeCardLk, isExpandMakeCardLk),
            if (_viewModel.makeCardLkDto != null &&
                _viewModel.makeCardLkDto!.documentNumber != null &&
                _viewModel.makeCardLkDto!.documentNumber!.isNotEmpty)
        ...
        child: Column(
          ...
          children: [
            if (_viewModel.personLkDto != null &&
                _viewModel.personLkDto!.pid != null &&
                _viewModel.personLkDto!.pid!.isNotEmpty)
              _buildActionTitle("ข้อมูลทะเบียนราษฎร",
                  _viewModel.requestPersonLk, isExpandPersonLk),
  ...
  Widget _buildPersonLkImage() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personLkImageDto != null &&
                _viewModel.personLkImageDto!.image != null &&
                _viewModel.personLkImageDto!.image!.isNotEmpty &&
                _viewModel.personLkImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.personLkImageDto == null ||
                    _viewModel.personLkImageDto!.image == null ||
                    _viewModel.personLkImageDto!.image!.isEmpty ||
                    _viewModel.personLkImageBytes == null ||
                    _viewModel.personLkImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  bool isExpandMakeCardLk() {
    return (_viewModel.makeCardLkDto != null &&
        _viewModel.makeCardLkDto!.documentNumber != null &&
        _viewModel.makeCardLkDto!.documentNumber!.isNotEmpty);
  }

  bool isExpandHouseLk() {
    return (_viewModel.houseLkDto != null &&
        _viewModel.houseLkDto!.houseID != null &&
        _viewModel.houseLkDto!.houseID!.isNotEmpty);
  }

  bool isExpandPersonLk() {
    return (_viewModel.personLkDto != null &&
        _viewModel.personLkDto!.pid != null &&
        _viewModel.personLkDto!.pid!.isNotEmpty);
  }
  ...
  Widget _buildAlienPersonDetail() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.alienPersonDto != null &&
                _viewModel.alienPersonDto!.personalID != null &&
                _viewModel.alienPersonDto!.personalID!.isNotEmpty)
              _buildActionTitle(
                  "ข้อมูลที่อยู่", _viewModel.requestHouseLk, isExpandHouseLk),
  ...
  Widget _buildAlienPersonImage() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.alienPersonImageDto != null &&
                _viewModel.alienPersonImageDto!.image != null &&
                _viewModel.alienPersonImageDto!.image!.isNotEmpty &&
                _viewModel.alienPersonImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.alienPersonImageDto == null ||
                    _viewModel.alienPersonImageDto!.image == null ||
                    _viewModel.alienPersonImageDto!.image!.isEmpty ||
                    _viewModel.alienPersonImageBytes == null ||
                    _viewModel.alienPersonImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildAlienWorkerImage() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.alienWorkerImageDto != null &&
                _viewModel.alienWorkerImageDto!.image != null &&
                _viewModel.alienWorkerImageDto!.image!.isNotEmpty &&
                _viewModel.alienWorkerImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.alienWorkerImageDto == null ||
                    _viewModel.alienWorkerImageDto!.image == null ||
                    _viewModel.alienWorkerImageDto!.image!.isEmpty ||
                    _viewModel.alienWorkerImageBytes == null ||
                    _viewModel.alienWorkerImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildPersonNonThaiImage() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personNonThaiImageDto != null &&
                _viewModel.personNonThaiImageDto!.image != null &&
                _viewModel.personNonThaiImageDto!.image!.isNotEmpty &&
                _viewModel.personNonThaiImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.personNonThaiImageDto == null ||
                    _viewModel.personNonThaiImageDto!.image == null ||
                    _viewModel.personNonThaiImageDto!.image!.isEmpty ||
                    _viewModel.personNonThaiImageBytes == null ||
                    _viewModel.personNonThaiImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildPersonNoRegisImage() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.personNoRegisImageDto != null &&
                _viewModel.personNoRegisImageDto!.image != null &&
                _viewModel.personNoRegisImageDto!.image!.isNotEmpty &&
                _viewModel.personNoRegisImageBytes != null)
              Container(
                padding: EdgeInsets.only(top: 10.h),
                child: Center(
                  child: Image.memory(
            ...
            if (!AppConstant.isMockUser &&
                (_viewModel.personNoRegisImageDto == null ||
                    _viewModel.personNoRegisImageDto!.image == null ||
                    _viewModel.personNoRegisImageDto!.image!.isEmpty ||
                    _viewModel.personNoRegisImageBytes == null ||
                    _viewModel.personNoRegisImageBytes!.length == 0))
              PrimaryCustomButton(
                "แสดงรูปภาพ",
  ...
  Widget _buildEmploymentDetail() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.employmentDto != null &&
                _viewModel.employmentDto!.ssoNum != null &&
                _viewModel.employmentDto!.ssoNum!.isNotEmpty)
              _buildActionTitle("ข้อมูลสถานพยาบาล", _viewModel.requestHospital,
                  isExpandHospital),
            if (_viewModel.hospitalDto != null &&
                _viewModel.hospitalDto!.hospitalCode != null &&
                _viewModel.hospitalDto!.hospitalCode!.isNotEmpty)
            ...
            if (_viewModel.employmentDto != null &&
                _viewModel.employmentDto!.accNo != null &&
                _viewModel.employmentDto!.accNo!.isNotEmpty &&
                _viewModel.employmentDto!.accBran != null &&
                _viewModel.employmentDto!.accBran!.isNotEmpty)
              _buildActionTitle("ข้อมูลนายจ้าง", _viewModel.requestEmployer,
                  isExpandEmployer),
            if (_viewModel.employerDto != null &&
                _viewModel.employerDto!.accNo != null &&
                _viewModel.employerDto!.accNo!.isNotEmpty)
  ...
  bool isExpandHospital() {
    return (_viewModel.hospitalDto != null &&
        _viewModel.hospitalDto!.hospitalCode != null &&
        _viewModel.hospitalDto!.hospitalCode!.isNotEmpty);
  }

  bool isExpandEmployer() {
    return (_viewModel.employerDto != null &&
        _viewModel.employerDto!.accNo != null &&
        _viewModel.employerDto!.accNo!.isNotEmpty);
  }
  // --- END Social ------
        ...
        child: Column(
          ...
          children: [
            if (_viewModel.warrantDto != null &&
                ...
                _viewModel.warrantDto!.orgCode!.isNotEmpty)
              _buildActionTitle(
                  "ข้อมูลรายละเอียดคดี${_viewModel.warrantDto!.caseType}",
                  _viewModel.requestWarrantCaseDetail,
                  isExpandedWarrantCaseDetail),
            if (_viewModel.warrantDto != null &&
                _viewModel.warrantDto!.orgCode != null &&
                _viewModel.warrantDto!.orgCode!.isNotEmpty)
            ...
            if (!AppConstant.isMockUser && (_viewModel.warrantDto != null))
              Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      if (_viewModel.warrantDto!.wrFileId != null &&
                          _viewModel.warrantDto!.wrFileId!.isNotEmpty &&
                          _viewModel.warrantDto!.wrFileId != '' &&
                          _viewModel.warrantDto!.wrFileId != '-')
                        PrimaryCustomButton(
                          "ดาวน์โหลดหมายจับ",
                        ...
                        SecondaryCustomButton(
                          "ดาวน์โหลดหมายจับ",
                          onPressed: () => _openDialogDetail("ดาวน์โหลดหมายจับ",
                              "ไม่พบข้อมูลไฟล์หมายจับในระบบ"),
                          icon: AppImage.ic_download,
                          isFullWidth: false,
                          padding: EdgeInsets.fromLTRB(10.w, 10.h, 10.w, 10.h),
                          //isEnabled:
                      ...
                      Material(
                        ...
                        child: InkWell(
                          ...
                          child: Container(
                            width: 45.w,
                            child: Center(
                              child: Image.asset(AppImage.ic_print,
                                  fit: BoxFit.contain,
                                  width: 30.w,
                                  height: 45.h),
                            ),
            ...
            if (!AppConstant.isMockUser && (_viewModel.warrantCRDDto != null))
              Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      if (_viewModel.warrantCRDDto!.warrant_doc != null &&
                          _viewModel.warrantCRDDto!.warrant_doc!.isNotEmpty)
                        PrimaryCustomButton(
                          "ดาวน์โหลดหมายจับ",
            ...
            if (!AppConstant.isMockUser &&
              ...
              Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      if (_viewModel.warrantCourtCRDDto!.warrant_doc != null &&
                          _viewModel
                              .warrantCourtCRDDto!.warrant_doc!.isNotEmpty)
                        PrimaryCustomButton(
                          "ดาวน์โหลดหมายจับ",
  ...
  bool isExpandedWarrantCaseDetail() {
    return (_viewModel.crimeCaseDetails != null ||
        _viewModel.trafficCaseDetails != null);
  }
  ...
  Widget _buildVehicleCrimeCase() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.crimeCaseVehicleDto != null &&
                _viewModel.crimeCaseVehicleDto!.orgCode != null &&
                _viewModel.crimeCaseVehicleDto!.orgCode!.isNotEmpty)
              _buildActionTitle(
                  "ข้อมูลรายละเอียดคดี",
                  _viewModel.requestVehicleCrimeCaseDetail,
                  isExpandedVehicleCrimeCaseDetail),
            if (_viewModel.crimeCaseVehicleDto != null &&
                _viewModel.crimeCaseVehicleDto!.orgCode != null &&
                _viewModel.crimeCaseVehicleDto!.orgCode!.isNotEmpty)
              if (_viewModel.crimeCaseDetails != null)
  ...
  Widget _buildVehicleTrafficCase() => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_viewModel.trafficCaseVehicleDto != null &&
                _viewModel.trafficCaseVehicleDto!.orgCode != null &&
                _viewModel.trafficCaseVehicleDto!.orgCode!.isNotEmpty)
              _buildActionTitle(
                  "ข้อมูลรายละเอียดคดี",
                  _viewModel.requestVehicleTrafficCaseDetail,
                  isExpandedVehicleTrafficCaseDetail),
            if (_viewModel.trafficCaseVehicleDto != null &&
                _viewModel.trafficCaseVehicleDto!.orgCode != null &&
                _viewModel.trafficCaseVehicleDto!.orgCode!.isNotEmpty)
              if (_viewModel.trafficCaseDetails != null)
        ...
        child: InkWell(
            ...
            onLongPress: () {
              FlutterClipboard.copy('${formDetail.desc}').then((value) =>
                  Fluttertoast.showToast(
                      msg: "คัดลอก ${formDetail.desc}",
                      toastLength: Toast.LENGTH_SHORT,
                      gravity: ToastGravity.BOTTOM));
            },
            onTap: () {
              if (formDetail.isShowDialog != null &&
                  formDetail.isShowDialog == true) {
                    ...
                    children: [
                      Expanded(
                        child: Text(
                          ...
                          style: formDetail.isHeader!
                              ? TextStyles.bodySemi.copyWith(
                                  color: AppColor.grey_text,
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  decoration: TextDecoration.underline)
                              : TextStyles.bodySemi
                                  .copyWith(color: AppColor.grey_text),
                        ),
                      ),
                        ...
                        Expanded(
                          child: Text(
                            ...
                            style: TextStyles.bodySemi.copyWith(
                                color: (formDetail.isAlert != null &&
                                        formDetail.isAlert == true)
                                    ? Colors.red
                                    : Colors.black,
                                decoration: (formDetail.isShowDialog != null &&
                                        formDetail.isShowDialog == true)
