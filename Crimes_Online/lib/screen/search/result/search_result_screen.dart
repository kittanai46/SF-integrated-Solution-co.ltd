...
class _SearchResultScreenState extends State<SearchResultScreen> {
  ...
  Future<void> _exportExcel(List<Result> resultArray, String? category) async {
    ...
    if (onValue != null) {
      ...
      Fluttertoast.showToast(
          msg: "ดาวน์โหลดไฟล์ .xlxs เก็บไว้ที่: ${path}",
  ...
  Future<void> _checkPermissionStorage(
      List<Result> resultArray, String? category) async {
    PermissionStatus storageStatus = await Permission.storage.status;
    StringUtils.log(
        '_checkPermissionStorage - storageStatus: ${storageStatus}');
    ...
    if (_permissions.length > 0) {
      if (storageStatus.isPermanentlyDenied) {
        ...
        _requestPermissionStorage(
            storageStatus, _permissions, resultArray, category);
      }
  ...
  Future<void> _requestPermissionStorage(
    ...
    StringUtils.log(
        '_requestPermissionStorage - storageStatus: ${storageStatus}');
  ...
  Future<void> _checkPermissionGallery(
      List<Result> resultArray, String? category) async {
    ...
    if (_permissions.length > 0) {
      if (photoStatus.isPermanentlyDenied) {
        ...
        _requestPermissionGallery(
            photoStatus, _permissions, resultArray, category);
      }
    } else if (_permissions.length == 0) {
      _exportExcel(resultArray, category);
    }
  ...
  void _selectItem(Result result, String category) {
    ScreenNavigator.navigateTo(Routes.searchDetail,
        param: SearchDetailScreenParam(
            _viewModel.searchType, category, result, _viewModel.userDto!));
  }

  Widget _getList(String? category, String? title, List<Result>? resultArray,
      String? errorMessage) {
    int count = 0;
            ...
            children: [
              Expanded(
                child: Text(
                  '${title}',
                  style:
                      TextStyles.bodySemi.copyWith(color: AppColor.blue_text),
                ),
              ),
              Text(
                '$count รายการ',
              ...
              if (resultArray != null &&
                  ...
                  resultArray[0].excelTitle != null &&
                  resultArray[0].excelTitle!.length > 0)
                Material(
                  color: Colors.transparent,
                  child: InkWell(
                    splashColor: Colors.white,
                    ...
                    child: Image.asset(AppImage.ic_share,
              ...
              if (resultArray != null &&
                  ...
                  resultArray[0].excelTitle != null &&
                  resultArray[0].excelTitle!.length == 0)
                Image.asset(AppImage.ic_share_grey,
                    fit: BoxFit.contain, width: 24.w),
            ],
          ),
          if (resultArray != null)
