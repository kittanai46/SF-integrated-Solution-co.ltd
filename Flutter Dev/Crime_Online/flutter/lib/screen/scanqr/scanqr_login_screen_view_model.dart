...
part 'scanqr_login_screen_view_model.g.dart';

class ScanQRLoginScreenViewModel = ScanQRLoginScreenViewModelBase with _$ScanQRLoginScreenViewModel;

abstract class ScanQRLoginScreenViewModelBase with Store {
  ScanQRLoginScreenViewModelBase();
  ...
  Future<bool> requestLoginScanQRCRIMES(String barcode) async {
    ...
    try {
      ...
      if(response.status!.isNotEmpty && response.status=='success'){
        StringUtils.log('LoginQR response.statusSuccess: ${response.status}');
        status = ActionStatus.success;
        successMessage = response.message!;
      }else{
        StringUtils.log('LoginQR response.status==>showErrorBefore: ${showError}');
        status = ActionStatus.error;
        errorMessage = response.message!;
        showError = true;
        StringUtils.log('LoginQR response.status==>showErrorAfter: ${showError}');
      }

    } on Exception catch (obj) {
      StringUtils.log('LoginQR response.status==>Exception: ${showError}');
      await _handleException(obj);
    }
