...
class SearchCriteriaScreenParam extends NavigationParameter {
  SearchCriteriaScreenParam(this.type, this.userDto, this.listPoliceStationDto, {this.pid, this.favoriteDto});
...
class _SearchCriteriaScreenState extends State<SearchCriteriaScreen> with AfterLayoutMixin<SearchCriteriaScreen>{
  ...
  Future<void> _initTutorial() async{
    if(widget.param.type == AppConstant.kSearchPerson && widget.param.favoriteDto == null){
      bool isRead = await TutorUtils.getTutorSPersonIsRead();
      if(!isRead){
        tutorSPersonWidget = TutorSPersonWidget(
            mKey: tutorKeySPerson, mContext: context,
            onNextTarget: (target) {
              if (target.identify == 'Target 1') {
                final Offset position = _getPositions(tutorKeySPerson.keyPersonParams2);
        ...
        Future.delayed(Duration(milliseconds: 500), () {
          _scrollController.animateTo(0,
              duration: Duration(milliseconds: 100), curve: Curves.linear);
          if(tutorSPersonWidget!=null) tutorSPersonWidget!.showTutorial(context);
        });
  ...
  Future<void> _openCamera() async {
    FocusScope.of(context).unfocus();
    final result = await ScreenNavigator.navigateTo(Routes.camera, param: CameraPageParam(cameras, false, UserDto(), ListPoliceStationDto(), FavoriteDto()));
  ...
  Future<void> _openGallery() async {
    ...
    final XFile? awaitImage = await ImagePicker().pickImage(source:ImageSource.gallery);
    ...
    if(_checkWords(results)){
      ...
      Fluttertoast.showToast(msg: "ไม่พบหมายเลขบัตรประชาชน", toastLength: Toast.LENGTH_SHORT, gravity: ToastGravity.BOTTOM);
    }
  ...
  bool _checkWords(String word) {
    ...
    RegExp regExp2 = new RegExp(r"(\d{1}) (\d{4}) (\d{5}) (\d{3})", multiLine: false);
    bool hasMatch = false;
  ...
  void _showResult(String foundText) {
    ...
    Fluttertoast.showToast(msg: "พบหมายเลข: $foundText", toastLength: Toast.LENGTH_LONG, gravity: ToastGravity.BOTTOM);
  ...
  Future<void> _checkPermissionCamera2() async{
    ...
    StringUtils.log('_checkPermissionCamera2 - microphoneStatus: ${microphoneStatus}');
  ...
  void _clearForm() {
    _tcIdCard.text = ''; _viewModel.setPid('');
    _tcFirstName.text = ''; _viewModel.setFirstName('');
    _tcLastName.text = ''; _viewModel.setLastName('');
    _tcOrgCode.text = ''; _viewModel.setOrgCode('');
    _tcCrimeCaseYear.text = ''; _viewModel.setCrimeCaseYear('');
    _tcCrimeCaseNo.text = ''; _viewModel.setCrimeCaseNo('');
    _tcPasId.text = ''; _viewModel.setPasId('');
    _tcNation.text = ''; _viewModel.setNation('');
    _tcProviceTc.text = ''; _viewModel.setProviceTc('');
    _tcIntitialNo.text = ''; _viewModel.setIntitialNo('');
    _tcNumber.text = ''; _viewModel.setNumber('');
    _tcStatus.text = ''; _viewModel.setStatus('');
    _tcEngineNo.text = ''; _viewModel.setEngineNo('');
    _licensenumber.text = ''; _viewModel.setLicenseNumber('');
    _tcProviceWr.text = ''; _viewModel.setProviceWr('');
    _tcTambonWr.text = ''; _viewModel.setTambonWr('');
    _tcAmphurWr.text = ''; _viewModel.setAmphurWr('');
    _tcCharge.text = ''; _viewModel.setCharge('');
    _tcCourt.text = ''; _viewModel.setCourt('');
    _tcWorkerId.text = ''; _viewModel.setWorkerId('');
    _tcPltcode1.text = ''; _viewModel.setPltcode1('');
    _tcPltcode2.text = ''; _viewModel.setPltcode2('');
    _tcPlate1Vh.text = ''; _viewModel.setPlate1Vh('');
    _tcPlate2Vh.text = ''; _viewModel.setPlate2Vh('');
    _tcProvcode.text = ''; _viewModel.setProvcode('');
    _tcVhtype.text = ''; _viewModel.setVhtype('');
    _tcNumbody.text = ''; _viewModel.setNumbody('');
    _tcBrncode.text = ''; _viewModel.setBrncode('');
    _tcAccno.text = ''; _viewModel.setAccno('');
    _tcAccbranch.text = ''; _viewModel.setAccbranch('');
    _tcPassportNo.text = ''; _viewModel.setPassportNoo('');
    _tcTmrunno.text = ''; _viewModel.setTmrunno('');
    _tcPlate1Tf.text = ''; _viewModel.setPlate1Tf('');
    _tcPlate2Tf.text = ''; _viewModel.setPlate2Tf('');
    _tcPlate3Tf.text = ''; _viewModel.setPlate3Tf('');
    _tcTicketId.text = ''; _viewModel.setTicketId('');

    // 🧼 เคลียร์รูปภาพด้วย
    _viewModel.setBase64Image('');

    _gunReg.text = ''; _viewModel.setGunReg('');

    _checkSetDefaultData();
  }

  void _openSelectDB() {
    ScreenNavigator.navigateTo(Routes.searchDB, param: SearchDBScreenParam(_viewModel.searchType, _viewModel.userDto!, _viewModel.scope, favoriteDto: _viewModel.favoriteDto));
  }
  ...
  Widget _buildLayout() => Container(
    ...
    child: Column(
      children: <Widget> [
        ...
        if(widget.param.favoriteDto==null || widget.param.favoriteDto!.choices == null)
    ...
    child: Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            if(_viewModel.scope.showInputPid || _viewModel.scope.showInputPidPlt1 || _viewModel.scope.showInputPidPlt2) SizedBox(height: 16.h,),
            if(_viewModel.scope.showInputPid || _viewModel.scope.showInputPidPlt1 || _viewModel.scope.showInputPidPlt2) _buildSectionImageChoose(),
            ...
            Column(
              ...
              children: [
                ...
                if((_viewModel.scope.showInputPid || _viewModel.scope.showInputNames)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                        && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormIdCard)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormFirstName)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormLastName)) ) ) _buildHeader("เงื่อนไขการค้นหาพื้นฐาน"),
                if(_viewModel.scope.base64Image!=null) _buildImagePreview(_viewModel.scope.base64Image!),
                if((_viewModel.scope.showInputPid || _viewModel.scope.showInputPidPlt1 || _viewModel.scope.showInputPidPlt2)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormIdCard)) ) _buildFormIdCard(),
                if((_viewModel.scope.showInputNames || _viewModel.scope.showInputNamesPlt1 || _viewModel.scope.showInputNamesPlt2)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormFirstName)) ) _buildFormFirstName(),
                if((_viewModel.scope.showInputNames || _viewModel.scope.showInputNamesPlt1 || _viewModel.scope.showInputNamesPlt2)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormLastName)) ) _buildFormLastName(),
              ]
            ),
            ...
            Column(
              ...
              children: [
                ...
                if(_viewModel.scope.showInputWorkerId
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormWorkerId)) ) _buildHeader("เงื่อนไข กลุ่ม: แรงงานต่างด้าว ตรวจคนเข้าเมือง"),
                if(_viewModel.scope.showInputWorkerId
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormWorkerId)) ) _buildFormWorkerId(),
                ...
                if(_viewModel.scope.showInputPasId
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPasId)) ) _buildFormPasId(),
                if((!_viewModel.scope.showInputPasId && _viewModel.scope.showInputPassportNo)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPasId)) ) _buildFormPassportNo(),
                // if(_viewModel.scope.showInputImmigration) _buildFormTmrunno(),
                if(_viewModel.scope.showInputPasId
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormNation)) ) _buildFormNation(),
            ...
            Column(
              ...
              children: [
                ...
                if((_viewModel.scope.showInputPidPlt1 || _viewModel.scope.showInputNamesPlt1 || _viewModel.scope.showInputPidPlt2 || _viewModel.scope.showInputNamesPlt2 || _viewModel.scope.showInputPlateProv)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                        && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormPltcode1)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPltcode2)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPlate1Vh)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPlate2Vh)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormProvcode)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormVhtype)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormNumbody)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormBrncode)) ) ) _buildHeader("เงื่อนไข กลุ่ม: กรมขนส่ง"),
                if((_viewModel.scope.showInputPidPlt1 || _viewModel.scope.showInputNamesPlt1)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPltcode1)) ) _buildFormPltcode1(),
                if((_viewModel.scope.showInputPidPlt2 || _viewModel.scope.showInputNamesPlt2)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormPltcode2)) ) _buildFormPltcode2(),

                if(_viewModel.scope.showInputPlateProv
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormPlate1Vh)
                        ||_viewModel.favoriteDto.choices!.contains(AppConstant.showFormIntitialNo)))
                    || _viewModel.scope.showInputFullNoPlate || _viewModel.scope.showInputPlateTicket) _buildFormPlate1Vh(),
                if(_viewModel.scope.showInputPlateProv
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormPlate2Vh)
                        ||_viewModel.favoriteDto.choices!.contains(AppConstant.showFormNumber)))
                    || _viewModel.scope.showInputFullNoPlate || _viewModel.scope.showInputPlateTicket) _buildFormPlate2Vh(),
                if(_viewModel.scope.showInputPlateProv
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormProvcode)
                        ||_viewModel.favoriteDto.choices!.contains(AppConstant.showFormProviceTc)))
                    || _viewModel.scope.showInputFullNoPlate || _viewModel.scope.showInputPlateTicket) _buildFormProvcode(),
                if(_viewModel.scope.showInputPlateProv
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormVhtype)) ) _buildFormVhtype(),

                if(_viewModel.scope.showInputNumBody
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormNumbody)) ) _buildFormNumbody(),
                if(_viewModel.scope.showInputNumBody
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormBrncode)) ) _buildFormBrncode(),
                ...
                if((_viewModel.scope.showInputFullNoPlate || _viewModel.scope.showInputFullEngineNo || _viewModel.scope.showInputPlateProv || _viewModel.scope.showInputNumBody)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                        && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormProviceTc)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormIntitialNo)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormNumber)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormStatus)
                            || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormEngineNo)) ) ) _buildHeader("เงื่อนไข กลุ่ม: ยานพาหนะในคดี"),
                ...
                if(_viewModel.scope.showInputFullEngineNo
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormEngineNo)) ) _buildFormEngineNo(),

                // Immigration, ForeignHouse
                if(_viewModel.scope.showInputPlateTicket
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormTicketId)) ) _buildHeader("เงื่อนไข กลุ่ม: ใบสั่งจราจร"),
                ...
                if(_viewModel.scope.showInputPlateTicket
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormTicketId)) ) _buildFormTicketId(),

                if((_viewModel.scope.showInputLicenseNumber)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                        && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormLicenseNumber)
                        ) ) ) _buildHeader("เงื่อนไข กลุ่ม: รถฉุกเฉิน"),
                if(_viewModel.scope.showInputLicenseNumber
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormLicenseNumber)) ) _buildFormLicenseNumber(),
            ...
            Column(
              ...
              children: [
                ...
                if((_viewModel.scope.showInputGunReg)
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                        && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormGunReg)) ) ) _buildHeader("เงื่อนไข กลุ่ม: ใบอนุญาต ป.4"),
                if(_viewModel.scope.showInputGunReg
                    && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormGunReg)) ) _buildFormGunReg(),
            ...
            if(_viewModel.scope.showInputFullcaseNo
                && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0)
                    && (_viewModel.favoriteDto.choices!.contains(AppConstant.showFormCrimeCaseNo)
                    || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormCrimeCaseYear)
                    || _viewModel.favoriteDto.choices!.contains(AppConstant.showFormOrgCode)) ) ) _buildHeader("เงื่อนไข กลุ่ม: คดีอาญา คดีจราจร หมายจับ"),
            if(_viewModel.scope.showInputFullcaseNo
                && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormCrimeCaseNo)) ) _buildFormCrimeCaseNo(),
            if(_viewModel.scope.showInputFullcaseNo
                && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormCrimeCaseYear)) ) _buildFormCrimeCaseYear(),
            if(_viewModel.scope.showInputFullcaseNo
                && ((_viewModel.favoriteDto.choices!.length==0) || (_viewModel.favoriteDto.choices!.length>0) && _viewModel.favoriteDto.choices!.contains(AppConstant.showFormOrgCode)) ) _buildFormOrgCode(),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              InkWell(
                onTap: () => _checkPermissionCamera2(), //_captureImageAndProcess(),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              InkWell(
                onTap: () =>  _checkPermissionGallery2(), //_pickImageFromGallery(),
                borderRadius: BorderRadius.circular(50),
                splashColor: Colors.grey.withOpacity(0.2),
                child: Container(
                  width: 40, // ขนาดวงกลมเล็กลง
  ...
  Widget _buildTitle() => Container(
    ...
    child: Text(
        'โปรดระบุข้อมูล เพื่อทำการสืบค้น',
  ...
  void _showDialogSelect(String title, List<String> array, TextEditingController edtController, bool isSearch, String searchHint, Function function) {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return FilterDialogContent(title, array, edtController, function, isSearch, searchHint);
        });
  }
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('หน่วยงาน', _viewModel.listPoliceStationName , _tcOrgCode, true, 'กรุณาระบุชื่อ หน่วยงาน', (index) => {
              _viewModel.setOrgCode(_viewModel.listPoliceStationID[index])
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
  ...
  Widget _buildFormCrimeCaseYear() => Container(
    child: Padding(
      ...
      child: CustomTextField(
        ...
        textInputAction: TextInputAction.next,
        onChanged: (text) => {
          _viewModel.setCrimeCaseYear(text)
        } ,
      ),
    ),
  );
  ...
  Widget _buildFormCrimeCaseNo() => Container(
    child: Padding(
      ...
      child: CustomTextField(
        ...
        textInputAction: TextInputAction.next,
        onChanged: (text) => {
          _viewModel.setCrimeCaseNo(text)
        } ,
      ),
    ),
  );
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('จังหวัด - ทะเบียนรถ', AppConstant.proviceTcKeys, _tcProviceTc, false, '', (index) => {
              _viewModel.setProvinceByIndex(index)
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('สถานะยานพาหนะในคดี', AppConstant.statusKeys, _tcStatus, false, '', (index) => {
  ...
  Widget _buildFormLicenseNumber() => Container(
    child: Padding(
      ...
      child: CustomTextField(
        ...
        textInputAction: TextInputAction.next,
        onChanged: (text) => {
          _viewModel.setLicenseNumber(text)
        } ,
      ),
    ),
  );
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ประเภทใบอนุญาตขับรถ', AppConstant.pltcode1Keys, _tcPltcode1, false, '', (index) => {
              _viewModel.setPltcode1(AppConstant.pltcode1Values[index])
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ประเภทใบอนุญาตขับรถ-ขนส่ง', AppConstant.pltcode2Keys, _tcPltcode2, false, '', (index) => {
              _viewModel.setPltcode2(AppConstant.pltcode2Values[index])
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ทะเบียนรถ (จังหวัด)', AppConstant.proviceTcKeys, _tcProvcode, false, '', (index) => {
              _viewModel.setProvinceByIndex(index)
            });
          },
          child: CustomTextField(
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ประเภทยานพาหนะ (กรมขนส่ง)', AppConstant.vhTypeKeys, _tcVhtype, false, '', (index) => {
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ยี่ห้อ (กรมขนส่ง)', AppConstant.brncodeKeys, _tcBrncode, false, '', (index) => {
              _viewModel.setBrncode(AppConstant.brncodeValues[index])
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
  ...
  Widget _buildFormPassportNo() => Container(
    child: Padding(
      ...
      child: CustomTextField(
        ...
        textInputAction: TextInputAction.next,
        onChanged: (text) => {
          _viewModel.setPassportNoo(text)
        } ,
      ),
    ),
  );
    ...
    child: Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(8.0),
          onTap: (){
            _showDialogSelect('ทะเบียนรถ (จังหวัด)', AppConstant.plate3TfKeys, _tcPlate3Tf, false, '', (index) => {
              _viewModel.setPlate3Tf(AppConstant.plate3TfValues[index])
            });
          },
          child: CustomTextField(
            keyboardType: TextInputType.text,
...
class FilterDialogContent extends StatefulWidget {
  FilterDialogContent(this.title, this.array, this.edtController, this.function, this.isSearch, this.searchHint, {Key? key}): super(key: key);
...
class _FilterDialogContentState extends State<FilterDialogContent> {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                CustomTextField(
                  keyboardType: TextInputType.text,
                  hintText: widget.searchHint.isNotEmpty ? widget.searchHint : 'ระบุคำค้นหา',
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => _getListItem(context, index, _choiceFilterArray, widget.edtController, widget.function),
                  ),
                ),
                SizedBox(height: 8.h,),
                PrimaryCustomButton(
  ...
  Widget _getListItem(BuildContext context, int index, List<String> array, TextEditingController edtController, Function function) {
    String selectValue = edtController.text;
    if(selectValue == null || selectValue == '') selectValue = AppConstant.blank;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.circular(8.0),
        onTap: (){
          _selectItem(context, array[index]/*, index*/, edtController, function);
        },
        child: Container(
  ...
  Future<void> _selectItem(BuildContext context, String value/*, int index*/, TextEditingController edtController, Function function) async{
    Navigator.pop(context);
