...
class SearchDBScreenViewModel = SearchDBScreenViewModelBase with _$SearchDBScreenViewModel;
...
abstract class SearchDBScreenViewModelBase with Store {
  ...
  Future<void> refreshCanAddFavorite() async{
    ...
    isEnableFavButton = favoriteList.length < AppConstant.maxFavorite && selectDbArray.length > 0;
  }
  ...
  Future<void> addFavorite(String title) async{
    ...
    if(paramScope.base64Image != null && paramScope.base64Image.isNotEmpty) choices.add(AppConstant.showFormImg);
    if(paramScope.pid != null && paramScope.pid.isNotEmpty) choices.add(AppConstant.showFormIdCard);
    if(paramScope.firstName != null && paramScope.firstName.isNotEmpty) choices.add(AppConstant.showFormFirstName);
    if(paramScope.lastName != null && paramScope.lastName.isNotEmpty) choices.add(AppConstant.showFormLastName);
    if(paramScope.workerId != null && paramScope.workerId.isNotEmpty) choices.add(AppConstant.showFormWorkerId);
    if((paramScope.pasId != null && paramScope.pasId.isNotEmpty) || (paramScope.passportNo != null && paramScope.passportNo.isNotEmpty) ) choices.add(AppConstant.showFormPasId);
    if(paramScope.nation != null && paramScope.nation.isNotEmpty) choices.add(AppConstant.showFormNation);
    if(paramScope.gunReg != null && paramScope.gunReg.isNotEmpty) choices.add(AppConstant.showFormGunReg);
    if(paramScope.proviceTc != null && paramScope.proviceTc.isNotEmpty) choices.add(AppConstant.showFormProviceTc);
    if(paramScope.intitialNo != null && paramScope.intitialNo.isNotEmpty) choices.add(AppConstant.showFormIntitialNo);
    if(paramScope.number != null && paramScope.number.isNotEmpty) choices.add(AppConstant.showFormNumber);
    if(paramScope.status != null && paramScope.status.isNotEmpty) choices.add(AppConstant.showFormStatus);
    if(paramScope.engineNo != null && paramScope.engineNo.isNotEmpty) choices.add(AppConstant.showFormEngineNo);
    if(paramScope.pltcode1 != null && paramScope.pltcode1.isNotEmpty) choices.add(AppConstant.showFormPltcode1);
    if(paramScope.pltcode2 != null && paramScope.pltcode2.isNotEmpty) choices.add(AppConstant.showFormPltcode2);
    if((paramScope.plate1Vh != null && paramScope.plate1Vh.isNotEmpty) || (paramScope.plate1Tf != null && paramScope.plate1Tf.isNotEmpty)) choices.add(AppConstant.showFormPlate1Vh);
    if((paramScope.plate2Vh != null && paramScope.plate2Vh.isNotEmpty) || (paramScope.plate2Tf != null && paramScope.plate2Tf.isNotEmpty)) choices.add(AppConstant.showFormPlate2Vh);
    if((paramScope.provcode != null && paramScope.provcode.isNotEmpty) || (paramScope.plate3Tf != null && paramScope.plate3Tf.isNotEmpty)) choices.add(AppConstant.showFormProvcode);
    if(paramScope.vhtype != null && paramScope.vhtype.isNotEmpty) choices.add(AppConstant.showFormVhtype);
    if(paramScope.numbody != null && paramScope.numbody.isNotEmpty) choices.add(AppConstant.showFormNumbody);
    if(paramScope.brncode != null && paramScope.brncode.isNotEmpty) choices.add(AppConstant.showFormBrncode);
    if(paramScope.ticketId != null && paramScope.ticketId.isNotEmpty) choices.add(AppConstant.showFormTicketId);
    if(paramScope.orgCode != null && paramScope.orgCode.isNotEmpty) choices.add(AppConstant.showFormOrgCode);
    if(paramScope.crimeCaseYear != null && paramScope.crimeCaseYear.isNotEmpty) choices.add(AppConstant.showFormCrimeCaseYear);
    if(paramScope.crimeCaseNo != null && paramScope.crimeCaseNo.isNotEmpty) choices.add(AppConstant.showFormCrimeCaseNo);
    if(paramScope.licensenumber != null && paramScope.licensenumber.isNotEmpty) choices.add(AppConstant.showFormLicenseNumber);
  ...
  void getListDB() {
    ...
    AllDatabase allDatabase = _apiClient.getListDB(searchType, userDto!, paramScope, false);
    ...
    try {
        ...
        if(category == AppConstant.dbInnerPersonDoAya) {
          ...
          if (paramScope.hasInputPid) {
            response = await ApiClient().getCrimeCasePersonByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputPasId) {
            response = await ApiClient().getCrimeCasePersonByPasId(paramScope.pasId, paramScope.nation, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNames) {
            response = await ApiClient().getCrimeCasePersonByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
        ...
        // TrafficCase Person
          ...
          if (paramScope.hasInputPid) {
            response = await ApiClient().getTrafficCasePersonByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputPasId) {
            response = await ApiClient().getTrafficCasePersonByPasId(paramScope.pasId, paramScope.nation, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNames) {
            response = await ApiClient().getTrafficCasePersonByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          } /*else if (paramScope.hasInputFullcaseNo) {
        ...
        // Warrant
          ...
          // 8 สืบค้นหมายจับ
          if (paramScope.hasInputPid) {
            response = await ApiClient().getWarrantByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNames) {
            response = await ApiClient().getWarrantByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputWrAddress) {
            response = await ApiClient().getWarrantByAddress(paramScope.proviceWr, paramScope.amphurWr, paramScope.tambonWr, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputCourt) {
            response = await ApiClient().getWarrantByCourt(paramScope.court, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputOrgCode) {
            response = await ApiClient().getWarrantByOrgCode(paramScope.orgCode, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // WarrantCRD
          ...
          // 8 สืบค้นหมายจับ
          if (paramScope.hasInputPid) {
            response = await ApiClient().getWarrantCRDByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNames) {
            response = await ApiClient().getWarrantCRDByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputWrAddress) {
            response = await ApiClient().getWarrantCRDByAddress(paramScope.proviceWr, paramScope.amphurWr, paramScope.tambonWr, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputCourt) {
            response = await ApiClient().getWarrantCRDByCourt(paramScope.court, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputOrgCode) {
            response = await ApiClient().getWarrantCRDByOrgCode(paramScope.orgCode, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // WarrantCourtCRD
          ...
          // 8 สืบค้นหมายจับ
          if (paramScope.hasInputPid) {
            response = await ApiClient().getWarrantCourtCRDByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNames) {
            response = await ApiClient().getWarrantCourtCRDByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // RedNotice
         ...
         if (paramScope.hasInputNames) {
            response = await ApiClient().getRedNoticeByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // FaceRec
          ...
          if (paramScope.hasInputImg) {
            response = await ApiClient().getPersonByFace(paramScope.base64Image,  lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // 2 = หาย
          ...
          if (paramScope.hasInputFullNoPlate) {
            response = await ApiClient().getCrimeCaseVehicleByFullNoPlate(paramScope.proviceTc, paramScope.intitialNo, paramScope.number, '0' , lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputFullEngineNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullEngineNo(paramScope.engineNo, '0', lastPosition.latitude, lastPosition.longitude);
          } /*else if (paramScope.hasInputFullcaseNo) {
        ...
        // TrafficCase Vehicle
          ...
          if (paramScope.hasInputFullNoPlate) {
            response = await ApiClient().getTrafficCaseVehicleByFullNoPlate(paramScope.proviceTc, paramScope.intitialNo, paramScope.number, '0', lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputFullEngineNo) {
            response = await ApiClient().getTrafficCaseVehicleByFullEngineNo(paramScope.engineNo, '0', lastPosition.latitude, lastPosition.longitude);
          } /*else if (paramScope.hasInputFullcaseNo) {
          ...
          if (paramScope.hasInputFullNoPlate) {
            response = await ApiClient().getCrimeCaseVehicleByFullNoPlate(paramScope.proviceTc, paramScope.intitialNo, paramScope.number, '2' , lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputFullEngineNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullEngineNo(paramScope.engineNo, '2', lastPosition.latitude, lastPosition.longitude);
          } /*else if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo);
          }*/
        }
          ...
          if (paramScope.hasInputFullNoPlate) {
            response = await ApiClient().getCrimeCaseVehicleByFullNoPlate(paramScope.proviceTc, paramScope.intitialNo, paramScope.number, '1' , lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputFullEngineNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullEngineNo(paramScope.engineNo, '1', lastPosition.latitude, lastPosition.longitude);
          } /*else if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo);
          }*/
        }
          ...
          if (paramScope.hasInputPlate) {
            response = await ApiClient().getEmergencyCarByPlate(paramScope.intitialNo, paramScope.number, paramScope.provcode, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputLicenseNumber) {
            response = await ApiClient().getEmergencyCarByLicenseNumber(paramScope.licensenumber, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        if(category == AppConstant.dbInnerInfoAya){
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getCrimeCaseByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }

        if(category == AppConstant.dbInnerInfoPersonSS){
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getCrimeCasePersonByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // TrafficCase
          ...
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getTrafficCaseByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }

        else if(category == AppConstant.dbInnerInfoPersonTC){
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getTrafficCasePersonByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }

        else if(category == AppConstant.dbInnerInfoVehicleCc){
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getCrimeCaseVehicleByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }

        else if(category == AppConstant.dbInnerInfoVehicleTc){
          if (paramScope.hasInputFullcaseNo) {
            response = await ApiClient().getTrafficCaseVehicleByFullcaseNo(paramScope.orgCode, paramScope.crimeCaseYear, paramScope.crimeCaseNo, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Person
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getPersonDetailByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getPersonDetailByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Worker
          ...
          if(paramScope.hasInputWorkerId) {
            response = await ApiClient().getForeignWorkerDetailByWorkerId(paramScope.workerId, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonLk
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getPersonInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getPersonInfoByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // DriverLicenceInfo
          ...
          if (paramScope.hasInputPidPlt1) {
            response = await ApiClient().getDriverLicenceInfoByPid(paramScope.pid, paramScope.pltcode1, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNamesPlt1) {
            response = await ApiClient().getDriverLicenceInfoByName(paramScope.firstName, paramScope.lastName, paramScope.pltcode1, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // DriverLicenceInfo
          ...
          if (paramScope.hasInputPidPlt2) {
            response = await ApiClient().getPublicLicenceInfoByPid(paramScope.pid, paramScope.pltcode2, lastPosition.latitude, lastPosition.longitude);
          } else if (paramScope.hasInputNamesPlt2) {
            response = await ApiClient().getPublicLicenceInfoByName(paramScope.firstName, paramScope.lastName, paramScope.pltcode2, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // VehicleInfo
          ...
          // กรมขนส่ง - ยานพาหนะ
          if(paramScope.hasInputPid) {
            response = await ApiClient().getVehicleInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getVehicleInfoByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputPlateProv) {
            response = await ApiClient().getVehicleInfoByPlate(paramScope.plate1Vh, paramScope.plate2Vh, paramScope.provcode, paramScope.vhtype, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNumBody) {
            response = await ApiClient().getVehicleInfoByNumBody(paramScope.numbody, paramScope.brncode, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputPassportNo) {
            response = await ApiClient().getVehicleInfoByPassport(paramScope.passportNo, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Employee Employment
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getEmployeeEmploymentByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Health
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getHealthInsuranceByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Immigration
          ...
          if(paramScope.hasInputPassportNo) {
            response = await ApiClient().getImmigrationByPassport(paramScope.passportNo, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getImmigrationByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Extension
          ...
          if(paramScope.hasInputPassportNo) {
            response = await ApiClient().getExtensionByPassport(paramScope.passportNo, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getExtensionByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Tm61
          ...
          if(paramScope.hasInputPassportNo) {
            response = await ApiClient().getTm61DetailByPassport(paramScope.passportNo, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getTm61DetailByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonTraffic
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getAllTickeList(paramScope.pid, "", "", "", "", lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getAllTickeListByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Gun
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getGunDetailByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
          if(paramScope.hasInputGunReg) {
            response = await ApiClient().getGunInfo(paramScope.gunReg, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // prisoner
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getPrisonerByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getPrisonerByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonForeign
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getForeignPersonInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonWorker
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getForeignWorkerInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonNonThai
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getPersonNonThaiInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // PersonNoRegis
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getPersonNoRegisInfoByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        //warrant court
          ...
          if(paramScope.hasInputPid) {
            response = await ApiClient().getWarrantCourtByPid(paramScope.pid, lastPosition.latitude, lastPosition.longitude);
          } else if(paramScope.hasInputNames) {
            response = await ApiClient().getWarrantCourtByName(paramScope.firstName, paramScope.lastName, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // Ticket
          ...
          if(paramScope.hasInputPlateTicket) {
            ...
            response = await ApiClient().getAllTickeList("", paramScope.plate1Tf, paramScope.plate2Tf, paramScope.plate3Tf, paramScope.ticketId, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // TODO: New API
          ...
          // ข้อมูลอาสาสมัครกู้ภัย
          if (paramScope.hasInputPlate) {
            response = await ApiClient().getCarFllByPlate(paramScope.plate1Vh, paramScope.plate2Vh, paramScope.provcode, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        // TODO: New API
          ...
          // ข้อมูลอาสาสมัครกู้ภัย
          if (paramScope.hasInputPlate) {
            response = await ApiClient().getCarAmlByPlate(paramScope.plate1Vh, paramScope.plate2Vh, paramScope.provcode, lastPosition.latitude, lastPosition.longitude);
          }
        }
        ...
        if(response != null) {
          ...
          // CrimeCase Person
          if(category == AppConstant.dbInnerPersonDoAya) {
            final crimeCasePersonResponse = response as GetCrimeCasePersonResponseDto;
            if(crimeCasePersonResponse!=null){
              if(crimeCasePersonResponse.status == AppConstant.statusSuccess){
                final listCrimeCasePerson = crimeCasePersonResponse.listCrimeCasePerson!.data;
                groupResult.resultArray = ResultMapper().mapCrimeCasePerson(listCrimeCasePerson);
              }else{
          ...
          // TrafficCase Person
            ...
            final trafficCasePersonResponse = response as GetTrafficCasePersonResponseDto;
            if(trafficCasePersonResponse!=null){
              if(trafficCasePersonResponse.status == AppConstant.statusSuccess){
                final listTrafficCasePerson = trafficCasePersonResponse.listTrafficCasePerson!.data;
                groupResult.resultArray = ResultMapper().mapTrafficCasePerson(listTrafficCasePerson, paramScope.pid, paramScope.pasId);
              }else{
                groupResult.resultArray = [];
          ...
          // WarrantCRD
            ...
            if(warrantCRDResponse!=null){
              if(warrantCRDResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapWarrantCRD(filterListWarrant);
              }else{
                groupResult.resultArray = [];
          ...
          // WarrantCourtCRD
            ...
            final warrantCourtCRDResponse = response as GetWarrantCourtCRDResponseDto;
            if(warrantCourtCRDResponse!=null){
              if(warrantCourtCRDResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapWarrantCourtCRD(filterListWarrant);
              }else{
                groupResult.resultArray = [];
          ...
          // RedNotice
            ...
            if(redNoticeResponse!=null){
              if(redNoticeResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapRedNotice(filterListWarrant);
              }else{
                groupResult.resultArray = [];
          ...
          // FaceRec
            ...
            if(faceResponse!=null){
              if(faceResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapFaceRec(filterListDate);
              }else{
                groupResult.resultArray = [];
          ...
          // CrimeCase Vehicle
            ...
            if(crimeCaseVehicleResponse!=null){
              if(crimeCaseVehicleResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapCrimeCaseVehicle(listCrimeCaseVehicle);
              }else{
                groupResult.resultArray = [];
          ...
          // TrafficCase Vehicle
            ...
            final trafficCaseVehicleResponse = response as GetTrafficCaseVehicleResponseDto;
            if(trafficCaseVehicleResponse!=null){
              if(trafficCaseVehicleResponse.status == AppConstant.statusSuccess){
                final listTrafficCaseVehicle = trafficCaseVehicleResponse.listTrafficCaseVehicle!.data;
                groupResult.resultArray = ResultMapper().mapTrafficCaseVehicle(listTrafficCaseVehicle);
              }else{
                groupResult.resultArray = [];
            ...
            if(crimeCaseVehicleResponse!=null){
              if(crimeCaseVehicleResponse.status == AppConstant.statusSuccess){
                final listCrimeCaseVehicle = crimeCaseVehicleResponse.listCrimeCaseVehicle!.data;
                groupResult.resultArray = ResultMapper().mapCrimeCaseVehicleLost(listCrimeCaseVehicle);
              }else{
                groupResult.resultArray = [];
                groupResult.errorMessage = crimeCaseVehicleResponse.message;
              }
          ...
          // CrimeCase
            ...
            if(crimeCaseResponse!=null){
              if(crimeCaseResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapCrimeCase(listCrimeCase);
              }else{
                groupResult.resultArray = [];
          ...
          // TrafficCase
            ...
            if(trafficCaseResponse!=null){
              if(trafficCaseResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapTrafficCase(listTrafficCase);
              }else{
                groupResult.resultArray = [];
          ...
          // PersonLk
            ...
            if(personLkResponse!=null){
              if(personLkResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapPersonLk(listPersonLk);
              }else{
                groupResult.resultArray = [];
          ...
          // AlienPerson
            ...
            if(alienPersonResponse!=null){
              if(alienPersonResponse.status == AppConstant.statusSuccess){
                final listAlienPerson = alienPersonResponse.listAlienPersonDto!.data;
                groupResult.resultArray = ResultMapper().mapAlienPerson(listAlienPerson);
              }else{
                groupResult.resultArray = [];
          ...
          // AlienWorker
            ...
            if(alienWorkerResponse!=null){
              if(alienWorkerResponse.status == AppConstant.statusSuccess){
                final listAlienWorker = alienWorkerResponse.listAlienWorkerDto!.data;
                groupResult.resultArray = ResultMapper().mapAlienWorker(listAlienWorker);
              }else{
                groupResult.resultArray = [];
          ...
          // PersonNonThai
            ...
            if(personNonThaiResponse!=null){
              if(personNonThaiResponse.status == AppConstant.statusSuccess){
                final listPersonNonThai = personNonThaiResponse.listPersonNonThaiDto!.data;
                groupResult.resultArray = ResultMapper().mapPersonNonThai(listPersonNonThai);
              }else{
                groupResult.resultArray = [];
          ...
          //PersonNoRegis
            ...
            if(personNoRegisResponse!=null){
              if(personNoRegisResponse.status == AppConstant.statusSuccess){
                final listPersonNoRegis = personNoRegisResponse.listPersonNoRegisDto!.data;
                groupResult.resultArray = ResultMapper().mapPersonNoRegis(listPersonNoRegis);
              }else{
                groupResult.resultArray = [];
          ...
          // DriverLicenceInfo
            ...
            final driverLicenseInfoResponse = response as DriverLicenseInfoResponseDto;
            if(driverLicenseInfoResponse!=null){
              if(driverLicenseInfoResponse.status == AppConstant.statusSuccess){
                final listDriverLicenseInfo = driverLicenseInfoResponse.listDriverLicenseInfo!.data;
                groupResult.resultArray = ResultMapper().mapDriverLicense(listDriverLicenseInfo);
              }else{
                groupResult.resultArray = [];
          ...
          // VehicleInfo
            ...
            if(vehicleInfoResponse!=null){
              if(vehicleInfoResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapVehicleInfo(listVehicleInfo);
              }else{
                groupResult.resultArray = [];
          ...
          // Employee Employment
            ...
            if(employmentResponse!=null){
              if(employmentResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapEmployment(listEmployment);
              }else{
                groupResult.resultArray = [];
          ...
          // Extension
            ...
            if(extensionResponse!=null){
              if(extensionResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapExtension(listExtension);
              }else{
                groupResult.resultArray = [];
          ...
          // Gun
            ...
            if(prisonerResponse!=null){
              if(prisonerResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapPrisoner(listPrisoner);
              }else{
                groupResult.resultArray = [];
          ...
          //Warrant Court
            ...
            if(warrantResponse!=null){
              if(warrantResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapWarrantCourt(filterListWarrant);
              }else{
                groupResult.resultArray = [];
                groupResult.errorMessage = warrantResponse.message;
              }
          ...
          /// RESPONSE - OUTER - case AppConstant.kSearchCar:
            ...
            if(vehicleInfoResponse!=null){
              if(vehicleInfoResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapVehicleInfo(listVehicle);
              }else{
                groupResult.resultArray = [];
                groupResult.errorMessage = vehicleInfoResponse.message;
              }
          ...
          //TODO: Response New API EmergencyCar
            ...
            if(emgcCarResponse!=null){
              if(emgcCarResponse.status == AppConstant.statusSuccess){
                ...
                groupResult.resultArray = ResultMapper().mapEmergencyCar((listEmgCar));
              }else{
                groupResult.resultArray = [];
  ...
  Future<void> _handleSearchException(Object obj, GroupResult groupResult) async{
