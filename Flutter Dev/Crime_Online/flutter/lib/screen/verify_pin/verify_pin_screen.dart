...
class VerifyPinScreenParam extends NavigationParameter {
  VerifyPinScreenParam(this.fromSessionTimeout);

  final bool fromSessionTimeout;
}

class VerifyPinScreen extends StatefulWidget {
  VerifyPinScreen(this.param, {Key? key}) : super(key: key);

  final VerifyPinScreenParam param;
...
class _VerifyPinScreenState extends State<VerifyPinScreen> {
  ...
  VerifyPinScreenViewModel _viewModel = VerifyPinScreenViewModel();
  ...
  Future<void> _checkEula() async{
    ...
    if(_viewModel.eulaDto != null){
      final isNewVersion = await KeyUtils.isNewEulaVersion(_viewModel.eulaDto!.version!);
      if(isNewVersion){
        ScreenNavigator.navigateReplaceTo(Routes.term, param: TermScreenParam(_viewModel.userDto!, _viewModel.eulaDto!));
      }else{
        ScreenNavigator.navigateReplaceTo(Routes.main, param: MainScreenParam(_viewModel.userDto!));
      }
    }else{
      ScreenNavigator.navigateReplaceTo(Routes.main, param: MainScreenParam(_viewModel.userDto!));
    }
